...
import { useNode, displayName, riskScore as nodeRiskScore } from "@entities/node";
import { useGraphStore } from "../model/graph-store";
import { useMarksStore } from "@shared/model/marks";
import { NODE_KIND_COLORS, NODE_KIND_COLORS_BY_KEY, riskBgClass } from "@shared/theme/tokens";
...
export function EntityInspector() {
  ...
  // Owned / High-Value marks now live in the explorer store (single persisted
  // source, key "agenthound-graph-marks"); selection stays on the graph store.
  ...
  const { data, isLoading } = useNode(selectedNodeId);
  ...
  const riskScore = nodeRiskScore(node);
