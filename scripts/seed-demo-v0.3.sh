#!/usr/bin/env bash
# AgentHound v0.3 demo seed.
#
# Brings up the v0.3 docker-compose lab (5 AI services + MCP discovery
...
#   - the new v0.3 :EXPOSES edge from Open WebUI to its Ollama backend
...
if [[ "$KEEP" -ne 1 ]]; then
    echo "[seed-demo-v0.3] tearing down any prior lab"
    docker compose -f "$LAB_COMPOSE" down --volumes --remove-orphans 2>/dev/null || true
fi

echo "[seed-demo-v0.3] starting lab"
docker compose -f "$LAB_COMPOSE" up -d --build

echo "[seed-demo-v0.3] waiting for services to be healthy"
sleep 8
...
echo "[seed-demo-v0.3] preloading Ollama with tinyllama + a fake fine-tune tag"
...
echo "[seed-demo-v0.3] running agenthound scan against $LAB_CIDR"
"$CMD_AGENTHOUND" scan "$LAB_CIDR" --output "$OUT_DIR/scan.json"

echo "[seed-demo-v0.3] running agenthound discover against $LAB_CIDR"
"$CMD_AGENTHOUND" discover "$LAB_CIDR" --output "$OUT_DIR/discover.json"

echo "[seed-demo-v0.3] running agenthound loot --type litellm"
...
echo "[seed-demo-v0.3] running agenthound loot --type ollama"
echo "AUTHORIZED" | "$CMD_AGENTHOUND" loot "$OLLAMA_HOST" \
    --type ollama \
    --engagement-id "$ENGAGEMENT_ID" \
    --output "$OUT_DIR/loot-ollama.json"

echo "[seed-demo-v0.3] ingesting envelopes"
for f in scan discover loot-litellm loot-ollama; do
    "$CMD_SERVER" ingest "$OUT_DIR/$f.json"
done

echo "[seed-demo-v0.3] complete."
echo "[seed-demo-v0.3] open http://localhost:8080 to explore the graph."
