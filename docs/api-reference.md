...
**There is no application-layer authentication.** AgentHound is single-user: the server runs on the operator's machine and is reachable only from a network the operator controls (loopback, VPN, SSH tunnel, Tailscale). Every endpoint listed here is open. See [`security.md`](security.md).

If you need to expose the server beyond loopback, do so at the network layer — never by punching a hole through the firewall directly to `0.0.0.0:8080`.
...
Execute raw Cypher against Neo4j. The endpoint is open; restrict access at the network layer.
