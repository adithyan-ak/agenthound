...
type MockGraphDB struct {
	...
	GetNodeResult *model.Node
	GetNodeEdges  []model.Edge
	GetNodeError  error

	ListNodesResult []model.Node
...
func (m *MockGraphDB) WriteEdges(ctx context.Context, edges []model.Edge, scanID string) (int, error) {
...
func (m *MockGraphDB) GetNode(ctx context.Context, objectID string) (*model.Node, []model.Edge, error) {
	m.record("GetNode", objectID)
	return m.GetNodeResult, m.GetNodeEdges, m.GetNodeError
}

func (m *MockGraphDB) ListNodes(ctx context.Context, kind string, limit int) ([]model.Node, error) {
