...
func New() (*Fingerprinter, error) {
	...
	for _, r := range all {
		if r.ServiceKind == "mlflow" {
			rule := r
			if errs := rules.ValidateFingerprint(rule); len(errs) > 0 {
				return nil, fmt.Errorf("mlflow rule invalid: %v", errs)
			}
			return &Fingerprinter{rule: &rule}, nil
		}
	}
	return nil, errors.New("mlflow fingerprint rule not found in builtin set")
}

func (f *Fingerprinter) Fingerprint(ctx context.Context, t action.Target) (*action.FingerprintResult, error) {
	if f.rule == nil {
		return nil, errors.New("mlflow fingerprinter: rule not loaded")
	}
	...
	if err != nil {
		slog.Debug("mlflow fingerprint probe error", "target", t.Address, "error", err)
	...
	endpoint := baseURL
	objectID := ingest.ComputeNodeID("MLflowServer", endpoint)
